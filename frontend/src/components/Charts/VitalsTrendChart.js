...
export default function VitalsTrendChart({ data, loading }) {
  ...
  console.log('Vitals Chart Data:', normalizedData);
        ...
        {VITAL_CONFIGS.map((v) => (
          <button
            ...
            style={{
              ...
              background: activeVitals.includes(v.key)
                ? `${v.color}20`
                : 'transparent',
              color: activeVitals.includes(v.key)
                ? v.color
                : 'var(--text-secondary)',
              cursor: 'pointer',
              transition: 'all 0.2s'
